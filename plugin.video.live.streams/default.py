...
#eseffair 06/11/2013 - novo epg 
def getepg(link):
        url=urllib.urlopen(link)
        source=url.read()
        url.close()
        soup = BeautifulSoup(source)
        programa1 = soup("li")[2].a["title"]
        horario1 = soup("li")[2].a.div.text
        programa2 = soup("li")[3].a["title"]
        horario2 = soup("li")[3].a.div.text
        #return "  - Agora: "+horario1+" "+programa1+" A seguir: "+horario2+" "+programa2
        return ' (%s - "%s" / %s - "%s")' % (horario1,programa1,horario2,programa2)
